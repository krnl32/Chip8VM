...
static int c8vm_init(const char *rompath);
static void c8vm_exit(void);
...
int main(int argc, char **argv)
{
	...
	if (c8vm_init(rompath) == -1) {
		fprintf(stderr, "c8vm_init failed\n");
		return -1;
	}
	...
	while (c8vm_platform->running) {
		...
		for (size_t i = 0; i < C8VM_CPU_CYCLES_PER_FRAME; i++) {
			if (c8vm_vm_cpu_cycle(c8vm_vm) == -1) {
				fprintf(stderr, "c8vm_vm_cpu_cycle failed\n");
				goto end;
			}
		}
...
static int c8vm_init(const char *rompath)
{
	...
	if (!c8vm_platform) {
		fprintf(stderr, "c8vm_platform_create failed\n");
		return -1;
	}
	...
	if (!c8vm_vm) {
		fprintf(stderr, "c8vm_vm_create failed\n");
		return -1;
	}

	// Setup Platform Audio
	if (c8vm_sound_init() == -1) {
		fprintf(stderr, "c8vm_sound_create failed\n");
		return -1;
	}
	...
	// Load Rom
	if (c8vm_vm_load_rom(c8vm_vm, rompath) == -1) {
		fprintf(stderr, "c8vm_vm_load_rom failed\n");
		return -1;
	}
