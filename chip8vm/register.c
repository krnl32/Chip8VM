...
uint8_t c8vm_register_read_vx(const struct c8vm_register *c8vm_register, uint8_t vreg)
{
	assert(vreg < C8VM_REGISTER_GPR_SIZE);
	return c8vm_register->v[vreg];
}

void c8vm_register_write_vx(struct c8vm_register *c8vm_register, uint8_t vreg, uint8_t data)
{
	assert(vreg < C8VM_REGISTER_GPR_SIZE);
